    ...
    map.current.on("load", () => {
      setTimeout(() => {
        ...
        if (map.current.getPitch() === 0) {
          map.current.easeTo({ pitch: 45, duration: 800 });
        }
    ...
    el.addEventListener("click", (e) => {
      ...
      if (isAssigningCameraRef.current && selectedPolygonIndexRef.current !== null) {
        ...
        if (polygon) {
          ...
          setCompletedPolygons((prev) => prev.map((poly, idx) => (idx === polygonIndex ? { ...poly, cameraId: id } : poly)));
    ...
    try {
      if (polygonPoints.length === 0) {
        ...
        map.current.addLayer({
          ...
          paint: { "circle-radius": 6, "circle-color": "#161b4cff", "circle-opacity": 0.5 },
        });
      } else {
        ...
        map.current.addLayer({
          ...
          type: "line",
          source: "polygon-guide",
          paint: { "line-color": "#161b4cff", "line-width": 2, "line-dasharray": [3, 3], "line-opacity": 0.7 },
        });
      }
    } catch (err) {}
  };
